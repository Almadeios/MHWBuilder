...
const SkillsPicker = ({ addSkill, addSlotFilter, showGroupSkillNames, chosenSkillNames }) => {
    ...
    const renderSlotFilters = grouped => {
        const filters = [1, 2, 3].map(x => {
            ...
            return <button type="button" key={`slot-filter-${x}`}
                ...
                {showIcons && <img
                    className="skills-search-bubble-icon" src={`images/slot${x}.png`} alt="" />}
                <span className="skills-search-bubble-text">
                    {`${x} Slot Deco Filter`}
                </span>
            </button>;
        });
    ...
    const renderSkill = skill => {
        ...
        const nameDiv = <span className={`skills-search-bubble-text ${highlightClass}-text`}>
            {skill.displayName}
        </span>;
    ...
    return <div className="skills-picker">
        <div style={{ display: "flex", gap: '8px' }}>
            ...
            {showIcons && <IconButton aria-label="Hide skill icons" title="Hide Icons"
                onClick={() => setShowIcons(!showIcons)}><AntiImageIcon /></IconButton>}
            {!showIcons && <IconButton aria-label="Show skill icons" title="Show Icons"
                onClick={() => setShowIcons(!showIcons)}>
                <ImageIcon className="image-icon" /></IconButton>}
            {!expanded && <IconButton aria-label="Expand skills box" title="Expand Skills Box"
                onClick={() => setExpanded(!expanded)}><ExpandIcon /></IconButton>}
            {expanded && <IconButton aria-label="Minimize skills box" title="Minimize Skills Box"
